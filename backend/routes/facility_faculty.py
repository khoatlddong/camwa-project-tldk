...
@router.post("/create", response_model=ApiResponse[FacilityFacultyResponse], status_code=201)
async def create_faculty(session: AsyncSessionDep, data: FacilityFacultyCreate):
...
async def get_all_faculties(session: AsyncSessionDep):
...
async def get_faculty(session: AsyncSessionDep, staff_id: str):
...
async def update_faculty(session: AsyncSessionDep, staff_id: str, data: FacilityFacultyUpdate):
...
async def delete_faculty(session: AsyncSessionDep, staff_id: str):
